...
import { useNotification, unstable_useContentManagerContext, unstable_useDocument } from "@strapi/strapi/admin";
...
import { ArrowLeft, Eye, EyeStriked, WarningCircle, Filter, CaretDown, ExternalLink, Link as Link$1 } from "@strapi/icons";
...
class ReferenceSet {
  ...
  describe() {
    ...
    for (const [, ref2] of this.refs) description2.push(ref2.describe());
    return description2;
  }
  toArray() {
...
React.forwardRef(
  ({ disabled = false, method, onSubmit, initialErrors, ...props }, ref2) => {
    ...
    const handleChange = useCallbackRef((eventOrPath, v2) => {
      ...
      const { type: type2, name: name2, id, value, options, multiple } = target;
...
const BooleanInput = forwardRef(
  ({ name: name2, required, label, hint, labelAction, ...props }, ref2) => {
    const { formatMessage } = useIntl();
    const field = useField(name2);
    const fieldRef = useFocusInputField(name2);
    const composedRefs = useComposedRefs(ref2, fieldRef);
    return /* @__PURE__ */ jsxs(Field.Root, { error: field.error, name: name2, hint, required, maxWidth: "320px", children: [
...
const CheckboxInput = forwardRef(
  ({ name: name2, required, label, hint, type: _type, ...props }, ref2) => {
    const field = useField(name2);
    const fieldRef = useFocusInputField(name2);
    const composedRefs = useComposedRefs(ref2, fieldRef);
    return /* @__PURE__ */ jsxs(Field.Root, { error: field.error, name: name2, hint, required, children: [
      /* @__PURE__ */ jsx(
        Checkbox,
        {
...
const DateInput = forwardRef(
  ({ name: name2, required, label, hint, labelAction, type: _type, ...props }, ref2) => {
    const { formatMessage } = useIntl();
    const field = useField(name2);
    const fieldRef = useFocusInputField(name2);
    const composedRefs = useComposedRefs(ref2, fieldRef);
...
const EnumerationInput = forwardRef(
  ({ name: name2, required, label, hint, labelAction, options = [], ...props }, ref2) => {
    const field = useField(name2);
    const fieldRef = useFocusInputField(name2);
    const composedRefs = useComposedRefs(ref2, fieldRef);
    return /* @__PURE__ */ jsxs(Field.Root, { error: field.error, name: name2, hint, required, children: [
...
const NumberInputImpl = forwardRef(
  ({ name: name2, required, label, hint, labelAction, type: type2, ...props }, ref2) => {
    const field = useField(name2);
    const fieldRef = useFocusInputField(name2);
    const composedRefs = useComposedRefs(ref2, fieldRef);
    return /* @__PURE__ */ jsxs(Field.Root, { error: field.error, name: name2, hint, required, children: [
...
const NotSupportedField = forwardRef(
  ({ label, hint, name: name2, required, type: type2, labelAction }, ref2) => {
    const { error } = useField(name2);
    const fieldRef = useFocusInputField(name2);
    const composedRefs = useComposedRefs(ref2, fieldRef);
    return /* @__PURE__ */ jsxs(Field.Root, { error, name: name2, hint, required, children: [
      /* @__PURE__ */ jsx(Field.Label, { action: labelAction, children: label }),
      /* @__PURE__ */ jsx(
        TextInput,
        {
...
const useShallowCompareEffect = (callback, dependencies2) => {
  React.useEffect(callback, useShallowCompareMemoize(dependencies2));
};
...
const PreviewButton = ({ url, isDraft }) => {
  const { formatMessage } = useIntl();
  const handleClick = () => {
    if (!url) return;
...
const PreviewButtonGroup = ({ config, document: document2 }) => {
  const isPublished = document2.status === "published";
  ...
  const params = Object.fromEntries(
    Object.entries(themeQuery).map(([key, value]) => [key, replacePlaceholders(value, document2)])
  );
  const url = `${themeUrl}?${new URLSearchParams(params)}`;
  return /* @__PURE__ */ jsxs(Wrapper, { direction: "column", gap: 2, children: [
    /* @__PURE__ */ jsx(PreviewButton, { url, isDraft: !isPublished }),
    /* @__PURE__ */ jsx(CopyButton, { url, isDraft: !isPublished })
  ] });
};
const Controls = () => {
  const context = unstable_useContentManagerContext();
  const entity = unstable_useDocument({
    documentId: context.id,
    model: context.model,
    collectionType: context.collectionType
  });
  const settings = useSettings();
  if (!settings || !Array.isArray(settings.contentTypes)) return /* @__PURE__ */ jsx(Fragment, {});
  if (context.isLoading || context.isCreatingEntry) {
    return /* @__PURE__ */ jsx(Fragment, {});
  }
  const contentType = settings.contentTypes.find(
    (contentType2) => contentType2.uid === context.contentType?.uid
  );
  if (!contentType) return /* @__PURE__ */ jsx(Fragment, {});
  if (!entity || entity.isLoading || !entity.document) return /* @__PURE__ */ jsx(Fragment, {});
  return /* @__PURE__ */ jsx(PreviewButtonGroup, { config: contentType, document: entity.document });
};
const index = {
  register(app) {
    app.registerPlugin({
  ...
  async registerTrads(app) {
    ...
    const importedTranslations = await Promise.all(
      locales.map((locale) => {
        return __variableDynamicImportRuntimeHelper(/* @__PURE__ */ Object.assign({ "./translations/en.json": () => import("../_chunks/en-yME2WcMq.mjs"), "./translations/es.json": () => import("../_chunks/es-BKMuHIX4.mjs"), "./translations/fr.json": () => import("../_chunks/fr-CbBK0CX2.mjs"), "./translations/ja.json": () => import("../_chunks/ja-BL-Apwu1.mjs"), "./translations/ru.json": () => import("../_chunks/ru-rm5Y4MOW.mjs"), "./translations/tr.json": () => import("../_chunks/tr-BAjCN18S.mjs") }), `./translations/${locale}.json`, 3).then(({ default: data }) => {
          return {
